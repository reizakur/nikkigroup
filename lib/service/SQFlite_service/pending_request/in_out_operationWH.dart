...
class PendingInOutWarehouseDbHelper {
  //static var
  static PendingInOutWarehouseDbHelper _databaseHelper;
  static Database _database;
  PendingInOutWarehouseDbHelper._createInstance();

  //Main Table Name
  String inOutTable = 'Pending_in_out_Warehouse_table';
  //id row
  ...
  factory PendingInOutWarehouseDbHelper() {
    // ignore: unnecessary_null_comparison
    if (_databaseHelper == null) {
      _databaseHelper = PendingInOutWarehouseDbHelper._createInstance();
    }
  ...
  Future<List<PendingInOutWarehouse>> getPendingInOutWarehouseList() async {
    var map = await getPendingInOutWarehouseMap();
    int count = map.length;
    List<PendingInOutWarehouse> accountRoleList = [];
    for (int i = 0; i < count; i++) {
      accountRoleList.add(PendingInOutWarehouse.frommapObject(map[i]));
    }
    return accountRoleList;
  }

  getPendingInOutWarehouseMap() async {
  ...
  //insert operation
  Future<int> insertData(PendingInOutWarehouse pendingInOutWarehouse) async {
    Database db = await this.database;
    var result = await db.insert(inOutTable, pendingInOutWarehouse.toMap());
  ...
  //delete operation
  Future<int> deleteAllPendingInOutWarehouse() async {
