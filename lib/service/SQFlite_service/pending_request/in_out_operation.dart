...
class PendingInOutDbHelper {
  //static var
  static PendingInOutDbHelper _databaseHelper;
  static Database _database;
  PendingInOutDbHelper._createInstance();

  //Main Table Name
  String inOutTable = 'Pending_in_out_table';
  //id row
  ...
  factory PendingInOutDbHelper() {
    // ignore: unnecessary_null_comparison
    if (_databaseHelper == null) {
      _databaseHelper = PendingInOutDbHelper._createInstance();
    }
  ...
  Future<Database> initializeDatabase() async {
    ...
    String path = directory.path + 'In_Out.db';
  ...
  Future<List<PendingInOut>> getPendingInOutList() async {
    var map = await getPendingInOutMap();
    int count = map.length;
    List<PendingInOut> accountRoleList = [];
    for (int i = 0; i < count; i++) {
      accountRoleList.add(PendingInOut.frommapObject(map[i]));
    }
  ...
  //insert operation
  Future<int> insertData(PendingInOut pendingInOut) async {
    Database db = await this.database;
    var result = await db.insert(inOutTable, pendingInOut.toMap());
  ...
  //delete operation
  Future<int> deleteAllPendingInOut() async {
